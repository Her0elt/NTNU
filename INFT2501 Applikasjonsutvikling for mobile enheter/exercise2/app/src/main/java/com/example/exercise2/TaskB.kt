...
class TaskB : AppCompatActivity() {
    ...
    private lateinit var upperLimitView: EditText
    ...
    private val startForResult =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            when (result.resultCode) {
                RESULT_OK -> {
                    if (result.data != null) {
                        val generatedNumber1 = result.data!!.getIntExtra("randomInt1", number1)
                        val generatedNumber2 = result.data!!.getIntExtra("randomInt2", number2)
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton(getString(R.string.generate_random_number)) { _, _ ->
                run {
                    ...
                    val intent = Intent(this, GenerateRandomInt::class.java)
                    intent.putExtra("upperBound", upperLimit)
